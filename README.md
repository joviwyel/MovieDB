...
# Connection Pooling
   ...
   * For different backend database, create different connection pool which databases are not necessarily master and slave. Depends on its purpose of connection, use different database. For read and write will use the master, the slave one for read only. When distributing task, if the task can be executed by both servers, then the distribution is random between both MySQL server and database. This is to split the heavy task load between the two databases.
...
|------------------------------------------------|------------------------------|----------------------------|-------------------------------------|---------------------------|--------------|
| Case 1: HTTP/1 thread                          | ![](images/scaled-case1-1.PNG)  | 95                          | 3.481594854                       | 3.109576045                        | The average TS and TJ time are short because of connection pooling. The average query time is long as there are limits set in connection pooling such as max idle time.   |
| Case 2: HTTP/10 threads                        | ![](images/scaled-case2-10.PNG)   | 94                        | 7.421240679              | 5.906030491                       | There are only 2 readers for 10 threads and 1 reader node reads 1 data entry at a time, so it has similar average query time as 1 thread. It takes slightly more time to reponse than 1 thread becasue there are more threads to be shared in the pool. However, the TS and TJ are shorter due to the sharing of 10 threads among servers.  |
| Case 3: HTTP/10 threads/No connection pooling  | ![](images/scaled-case3-10.PNG)   | 95                         | 6.578229258                                  | 4.629758364                        | Without connection pooling and having 10 threads should take longer to response because threads are created and closed every time. If we increment the number of threads by 10 times, the effect might be clearer.            |
